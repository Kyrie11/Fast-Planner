...
#include <utility>
#include "dynamic_voronoi/vector2d.h"
...
class KinodynamicAstar {
 private:
  ...
  /* map */
  double resolution_, inv_resolution_, time_resolution_, inv_time_resolution_, cellResolution;
  ...
  void stateTransit(Eigen::Matrix<double, 6, 1>& state0,
                    ...
                    double tau);
  inline float clamp(float n, float lower, float upper) {return std::max(lower, std::min(n, upper));}
  ...
  float voroObsDMax = 4;
  //权重系数
  float alpha = 0.1;//维诺图权重系数
  float wObstacle = 0.2;//障碍物权重系数
  float wVoronoi = 0;//维诺项权重系数
  float wCurvature = 0;//曲折项权重系数
  float wSmoothness = 0.2;//平滑项权重系数
  float kappaMax =1.f / 36*1.1;//非完整飞行器的最大可能曲率
 ...
 public:
  ...
  void init();
  void reset();
  int search(DynamicVoronoi voronoi, Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  void smoothPath(DynamicVoronoi& voronoi);//通过维诺图对路径进行平滑
  Vector2D smoothenessTerm(Vector2D xim2, Vector2D xim1, Vector2D xi, Vector2D xip1, Vector2D xip2);
  inline bool isCusp(std::vector<PathNodePtr> path, int i);//判断两个点是否相交
  //返回离最近的障碍物的梯度方向
