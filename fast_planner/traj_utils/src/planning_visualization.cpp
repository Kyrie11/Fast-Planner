...
PlanningVisualization::PlanningVisualization(ros::NodeHandle& nh) {
  ...
  voronoi_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/voronoi_path", 20);
  pubs_.push_back(voronoi_pub_);

  voronoiNode_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/voronoi_nodes", 20);
  pubs_.push_back(voronoiNode_pub_);
...
void PlanningVisualization::drawVoronoi(vector<Eigen::Vector3d> points) 
{
  if (points.size() == 0) return;
  displaySphereList(points, 0.2, Eigen::Vector4d(1,0,1,1), 100, 6);
}
