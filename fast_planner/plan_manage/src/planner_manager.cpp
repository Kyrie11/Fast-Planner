...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  bool use_geometric_path, use_kinodynamic_path, use_topo_path, use_voronoi_path, use_optimization, use_active_perception;
  ...
  nh.param("manager/use_voronoi_path", use_voronoi_path, false);
  nh.param("manager/use_optimization", use_optimization, false);

  voronoi_path_pub_ = nh.advertise<visualization_msgs::Marker>("/planning_vis/voronoi_origin", 20);
  ...
  if (use_voronoi_path) {
    std::cout<<"使用voronoibubble \n";
    voronoi_path_finder_.reset(new VoronoiBubble(nh));
    //voronoi_path_finder_->setParam(nh);
    voronoi_path_finder_->setEnvironment(edt_environment_);        
    voronoi_path_finder_->init();
    std::cout<<"voronoi_path_finder init 成功\n";
  } 
...
bool FastPlannerManager::kinodynamicReplan(DynamicVoronoi voronoi, Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  int status = kino_path_finder_->search(voronoi, start_pt, start_vel, start_acc, end_pt, end_vel, true);
  ...
  if (status == KinodynamicAstar::NO_PATH) {
    ...
    kino_path_finder_->reset();
    status = kino_path_finder_->search(voronoi, start_pt, start_vel, start_acc, end_pt, end_vel, false);
...
void FastPlannerManager::displayVoronoiPath(vector<Eigen::Vector3d> points) 
{
...
bool FastPlannerManager::voronoiReplan(Eigen::Vector3d start_pt,  Eigen::Vector3d start_vel, 
				       Eigen::Vector3d start_acc, Eigen::Vector3d end_pt, 
				       Eigen::Vector3d end_vel, DynamicVoronoi voronoiDiagram_, vector<Eigen::Vector3d>& points) {
  std::cout << "[voronoi replan]: --------------" << std::endl;
  cout << "start:" <<start_pt.transpose() << "," << start_vel.transpose() << ","
       << start_acc.transpose() << "\ngoal:" << end_pt.transpose() << "," << end_vel.transpose()
       << endl;
  ...
  int startVoronoiX = ceil(start_pt[0] - voronoiDiagram.originX);
  int startVoronoiY = ceil(start_pt[1] - voronoiDiagram.originY);
  int endVoronoiX   = ceil(end_pt[0] - voronoiDiagram.originX);
  int endVoronoiY   = ceil(end_pt[1] - voronoiDiagram.originY);
  ...
  t1 = ros::Time::now();
  voronoi_path_finder_->reset();
  std::cout << "[voronoi replan]: 开始寻路\n";
  //int status = voronoi_path_finder_->search2(start_pt, start_vel, start_acc, end_pt, end_vel, voronoiDiagram, true);
  int status = voronoi_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, voronoiDiagram);
  if (status == VoronoiBubble::NO_PATH)
  {
    cout << "[voronoi replan]: voronoi search fail!" << endl;
    voronoi_path_finder_->reset();
    //status = voronoi_path_finder_->search2(start_pt, start_vel, start_acc, end_pt, end_vel, voronoiDiagram, true);
    status = voronoi_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, voronoiDiagram);
    if (status == VoronoiBubble::NO_PATH) {
      cout << "[voronoi replan]: Can't find path." << endl;
      return false;
    } else {
      cout <<"[voronoi replan]: retry search success." << endl;
    }
  } else {
    cout << "[voronoi replan]: voronoi search success." << endl;
  }
  plan_data_.voronoi_path_ = voronoi_path_finder_->getVoronoiTraj(0.01);
  
  t_search = (ros::Time::now() -  t1).toSec();
  std::cout<<"总共耗时"<<t_search<<std::endl;
  //double ts = pp_.ctrl_pt_dist / pp_.max_vel_;
  double ts = 0.1;
  vector<Eigen::Vector3d> point_set, start_end_derivatives;
  //end_vel = plan_data_.voronoi_path_.back().state.tail(3);
  voronoi_path_finder_->getSamples(ts, point_set, start_end_derivatives);
  //points = point_set;
  displayVoronoiPath(point_set);
  //displayVoronoiPath(voronoi_path_finder_->path_nodes_);
  Eigen::MatrixXd ctrl_pts;
  NonUniformBspline::parameterizeToBspline(ts, point_set, start_end_derivatives, ctrl_pts);
  NonUniformBspline init(ctrl_pts, 3, ts);
  ...
  if (status != VoronoiBubble::REACH_END) {
    cost_function |= BsplineOptimizer::ENDPOINT;
  }
